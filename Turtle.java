...
// Bob and friends :)
class BobTheTurtle implements Runnable {
    public void run() {
        // Happy drawing, Bob!
        ...
        for (int i = 0; i < 5; i++) {
            bob.forward(100);
            bob.right(144); // Angle for the star shape.
        }
    }
}

class BucellatiTheTurtle implements Runnable {
    public void run() {
        // Bruno Bucellati
        Turtle Bucellati = new Turtle();
        Bucellati.width(5);
        Bucellati.speed(10);

        // Bucellati draws a spiral.
        Bucellati.penColor("purple");
        Bucellati.up();
        Bucellati.setPosition(-100, 0);
        Bucellati.down();

        for (int i = 0; i < 100; i++) {
            Bucellati.forward(i * 2); // Gradually increasing forward distance.
            Bucellati.right(30); // Turn right by 30 degrees to form a spiral.
        }
    }
}

class GiornoTheTurtle implements Runnable {
    public void run() {
        // Giorno Giovanna
        Turtle Giorno = new Turtle();
        Giorno.width(5);
        Giorno.speed(20);

        // Giorno draws an equilateral triangle.
        Giorno.penColor("green");
        Giorno.up();
        Giorno.setPosition(100, -150);
        Giorno.down();

        for (int i = 0; i < 3; i++) {
            Giorno.forward(150); // Side length of the triangle.
            Giorno.left(120); // Angle to turn for equilateral triangle.
        }
    }
}
...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    {
        // Bob and friends
        Thread bob = new Thread(new BobTheTurtle());
        Thread bucellati = new Thread(new BucellatiTheTurtle());
        Thread giorno = new Thread(new GiornoTheTurtle());
